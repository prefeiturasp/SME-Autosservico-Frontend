...
vi.mock("@/hooks/useOcorrenciaInterpessoal", () => ({
  useOcorrenciaInterpessoal: () => mockQueryResult,
}));

import OcorrenciaInterpessoalCard from "./OcorrenciaInterpessoalCard";

describe("<OcorrenciaInterpessoalCard />", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockQueryResult = {
      data: undefined,
  ...
  it("sem systemName mostra placeholder", () => {
    render(<OcorrenciaInterpessoalCard />);
    expect(screen.getByText("Selecione um projeto")).toBeInTheDocument();
  });

  it("loading mostra skeletons", () => {
    mockQueryResult = { ...mockQueryResult, isLoading: true };
    render(<OcorrenciaInterpessoalCard systemName="GIPE" />);
  ...
  it("erro mostra mensagem e botão de retry", async () => {
    ...
    expect(
      screen.getByText("Não foi possível carregar as ocorrências interpessoais."),
    ).toBeInTheDocument();
  ...
  it("sucesso renderiza as linhas visíveis e trunca as demais atrás do 'Ver mais'", async () => {
    ...
    expect(
      screen.getByText("Ocorrência com objeto sem ameaça"),
    ).toBeInTheDocument();
