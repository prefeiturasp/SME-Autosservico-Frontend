...
vi.mock("@/hooks/useOcorrenciasPorCategoria", () => ({
  useOcorrenciasPorCategoria: () => mockQueryResult,
}));

import OcorrenciasPorCategoriaCard from "./OcorrenciasPorCategoriaCard";

describe("<OcorrenciasPorCategoriaCard />", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockQueryResult = {
      data: undefined,
  ...
  it("sem systemName mostra placeholder", () => {
    render(<OcorrenciasPorCategoriaCard />);
    expect(screen.getByText("Selecione um projeto")).toBeInTheDocument();
  });

  it("loading mostra skeletons", () => {
    mockQueryResult = { ...mockQueryResult, isLoading: true };
    render(<OcorrenciasPorCategoriaCard systemName="GIPE" />);
  ...
  it("erro mostra mensagem e botão de retry", async () => {
    ...
    expect(
      screen.getByText("Não foi possível carregar as ocorrências por categoria."),
    ).toBeInTheDocument();
