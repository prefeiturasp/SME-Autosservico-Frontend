...
describe("fetchDatabaseStatus", () => {
    ...
    it("zabbixRpc retorna array vazio → lastvalue usa fallback '' → available false (MySQL)", async () => {
        mockZabbixRpc.mockResolvedValueOnce([]);

        const result = await fetchDatabaseStatus("Portal Educação");
        expect(result.hasDatabase).toBe(true);
        expect(result.instances[0].available).toBe(false);
        expect(result.instances[0]).not.toHaveProperty("role");
    });

    it("erro na chamada Zabbix para sistema com role → retorna role no resultado", async () => {
        mockZabbixRpc
        ...
        expect(result.instances[0].available).toBe(false);
        expect(result.instances[0].role).toBe("escrita");
        expect(result.instances[1].available).toBe(false);
        expect(result.instances[1].role).toBe("leitura");
    });
});
