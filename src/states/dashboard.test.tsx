...
describe("useDashboardStore (Zustand)", () => {
    ...
    it("deve atualizar o activeItem com setActiveItem", () => {
        ...
        expect(useDashboardStore.getState().activeItem).toEqual(
            mockSidebarItem,
        );
    });
    ...
    it("deve limpar o activeProject com clearActiveProject", () => {
        const { setActiveProject, clearActiveProject } =
    ...
    it("alterar activeItem não deve alterar activeProject", () => {
        const { setActiveItem, setActiveProject } =
            useDashboardStore.getState();
        setActiveProject(mockProject);
        setActiveItem(mockSidebarItem);
    ...
    it("deve atualizar activeTab com setActiveTab", () => {
        const { setActiveTab } = useDashboardStore.getState();
        setActiveTab("analytics");
        expect(useDashboardStore.getState().activeTab).toBe("analytics");
    });

    it("deve atualizar activePeriod com setActivePeriod", () => {
        const { setActivePeriod } = useDashboardStore.getState();
        setActivePeriod("7-dias");
        expect(useDashboardStore.getState().activePeriod).toBe("7-dias");
    });
});
