...
vi.mock("@/hooks/useOrdemInscricaoPorGanhador", () => ({
  useOrdemInscricaoPorGanhador: ({ period }: { period: string }) => {
...
describe("<OrdemInscricaoPorGanhadorCard />", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("sem systemName mostra placeholder", () => {
    render(<OrdemInscricaoPorGanhadorCard />);
    expect(screen.getByText("Selecione um projeto")).toBeInTheDocument();
  });

  it("usa 'dia' como período inicial", () => {
    render(<OrdemInscricaoPorGanhadorCard systemName="Intranet" />);
  ...
  it("erro mostra mensagem e botão de retry", async () => {
    ...
    expect(
      screen.getByText(
        "Não foi possível carregar os resultados de ordens de inscrição por ganhador."
      )
    ).toBeInTheDocument();
  ...
  it("sucesso renderiza as linhas mockadas", () => {
    mockQueryResult = {
      ...mockQueryResult,
      data: [{ label: "Servidores", value: 234 }],
    };
    render(<OrdemInscricaoPorGanhadorCard systemName="Intranet" />);
