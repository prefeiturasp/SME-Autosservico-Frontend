...
describe("useFetchDisponibilidadeDosAmbientesProducao", () => {
  ...
  it("não chama fetch quando projectName vazio (enabled=false)", async () => {
    const r = renderHook(
      () => useFetchDisponibilidadeDosAmbientesProducao(""),
      { wrapper }
    );
  ...
  it("retorna dados de sucesso", async () => {
    ...
    const { result } = renderHook(
      () => useFetchDisponibilidadeDosAmbientesProducao("Portal Educação", "Zabbix server"),
      { wrapper }
    );
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining("/api/disponibilidade-dos-ambientes/producao?"),
      { cache: "no-store" }
    );
  });
  ...
  it("propaga erro quando response !ok", async () => {
    ...
    const { result } = renderHook(
      () => useFetchDisponibilidadeDosAmbientesProducao("Portal Educação"),
      { wrapper }
    );
