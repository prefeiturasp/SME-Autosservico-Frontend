...
describe("<UsersByPageCard />", () => {
    ...
    it("renderiza apenas as 5 primeiras páginas por padrão", () => {
        ...
        expect(
            screen.queryByTestId("users-by-page-row-6"),
    ...
    it("filtra a lista pela página selecionada", async () => {
        ...
        expect(
            screen.queryByTestId("users-by-page-row-2"),
        ).not.toBeInTheDocument();
    });
    ...
    it("ordena por 'Agora' em ordem ascendente no primeiro clique", async () => {
        ...
        await userEvent.click(
            screen.getByTestId("users-by-page-sort-currentUsers"),
        );

        expect(screen.getByTestId("users-by-page-row-1")).toHaveTextContent(
            "/b",
        );
        expect(screen.getByTestId("users-by-page-row-2")).toHaveTextContent(
            "/c",
        );
        expect(screen.getByTestId("users-by-page-row-3")).toHaveTextContent(
            "/a",
        );
    });
    ...
    it("fillPercentage é 0 quando todos os currentUsers são 0 (maxCurrent = 0)", () => {
        mockQueryResult = {
            ...mockQueryResult,
            data: {
                system: "SigPAE",
                pages: [
                    { path: "/pagina-1", currentUsers: 0, averageUsers: 100 },
                    { path: "/pagina-2", currentUsers: 0, averageUsers: 200 },
                ],
            },
        };
        render(<UsersByPageCard systemName="SigPAE" />);
